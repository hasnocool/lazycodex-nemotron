...
function selectionFromArgs(args) {
  ...
  const route = args.includes("--openrouter") ? RECOMMENDED_ROUTES.freeReasoning : RECOMMENDED_ROUTES.primaryReasoning
  const [providerId, ...parts] = route.split("/")
  return { provider: providerId, model: parts.join("/") }
}
...
export async function runDoctor({ args = [], fetchImpl = fetch } = {}) {
  ...
  const wireApi = parseOption(args, "--wire-api") || provider?.wire_api || "responses"
  const config = providerHealthConfig(selection.provider, selection.model, { base_url: baseUrl, env_key: envKey, wire_api: wireApi })
  ...
  try {
    ...
    return { ...config, status: "ok", responseId: result?.id || null }
  } catch (error) {
    return { ...config, status: "failed", message: error instanceof Error ? error.message : String(error) }
  }
}
