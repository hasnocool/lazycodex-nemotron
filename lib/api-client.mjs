...
function resolveDefinition(providerId, overrides = {}) {
  ...
  if (!overrides.base_url) {
    throw new Error(`Unknown provider ${providerId}; a base URL is required`)
  }
  return providerDefinition(providerId, overrides)
}

export async function callChat({ provider, model, apiKey, messages, temperature, maxTokens, signal, baseUrl, envKey }) {
  ...
  const response = await fetch(apiUrl(definition, "chat"), {
    method: "POST",
...
export async function callResponses({ provider, model, apiKey, input, instructions, signal, baseUrl, envKey }) {
  ...
  const response = await fetch(apiUrl(definition, "responses"), {
    method: "POST",
    headers: authHeaders(definition, apiKey),
    body: JSON.stringify(payload),
    signal,
