...
function upsertRootKey(source, key, value) {
  ...
  const keyRegex = new RegExp(`^${key.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&")}\\s*=.*$`, "u")
...
export async function configureCodexRouting({ args = [], dryRun = false } = {}) {
  ...
  const profileNames = selections.length > 1
    ? selections.map((selection) => `lazycodex-${selection.provider}`)
    : [requestedProfile]

  for (let index = 0; index < selections.length; index += 1) {
    next = upsertSection(next, `[profiles.${profileNames[index]}]`, profileBlock(profileNames[index], selections[index]))
  }
