...
const LEGACY_STALE_MODELS = new Set(["nvidia/nemotron-3-nano"])
...
async function migrateLegacyAgentModels(codexHome, model, { dryRun = false } = {}) {
  ...
  for (const entry of entries) {
    ...
    const next = original.replace(/^model\s*=\s*"([^"]+)"\s*$/gmu, (line, currentModel) => {
      return LEGACY_STALE_MODELS.has(currentModel) ? `model = ${JSON.stringify(model)}` : line
    })
...
export async function configureCodexRouting({ args = [], dryRun = false } = {}) {
  ...
  const migrationResult = await migrateLegacyAgentModels(codexHome, primary.model, { dryRun })
  const explicitPatchResult = patchAgents
    ? await patchCodexAgents(codexHome, primary.model, { dryRun })
    : { changed: [], backups: [] }
  const agentResult = {
    changed: [...new Set([...migrationResult.changed, ...explicitPatchResult.changed])],
    backups: [...migrationResult.backups, ...explicitPatchResult.backups],
  }
  ...
  return {
    ...
    agentResult,
    patchAgents,
    migratedLegacyAgents: migrationResult.changed,
...
export function codexModelHelp() {
  return [
    ...
    "",
    "Legacy cleanup:",
    "  configure/install automatically migrate the retired nvidia/nemotron-3-nano agent model to the active model.",
    "",
    "Live API checks:",
